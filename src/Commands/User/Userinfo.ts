...
export const command: Command = {
    ...
    run: async (client, message, args) => {
        ...
        let user: User = message.mentions.users.first() || client.users.cache.get(args[0]) || message.author;
        ...
        let embed: MessageEmbed = new MessageEmbed()
            ...
            .addField('Created At', `${moment(user.createdAt).format('LLLL')} (${checkDays(user.createdAt)})`)
            .addField('Joined At', `${moment(member.joinedAt).format('LLLL')} (${checkDays(member.joinedAt)})`)
